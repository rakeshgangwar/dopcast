"""
Test script to fetch a transcript for a specific YouTube video using OpenAI API.
"""

import os
import sys
import json
from datetime import datetime

# Add the project root to the Python path
sys.path.insert(0, os.path.abspath(os.path.dirname(__file__)))

from agents.research.tools.youtube_transcript_tool import YouTubeTranscriptTool

def main():
    # Create output directory if it doesn't exist
    output_dir = os.path.join(os.path.dirname(__file__), "output")
    data_dir = os.path.join(output_dir, "data")
    os.makedirs(data_dir, exist_ok=True)

    # Get OpenAI API key from environment variable
    openai_api_key = os.environ.get("OPENAI_API_KEY", "")
    if not openai_api_key:
        print("Error: OPENAI_API_KEY environment variable not set.")
        print("Please set it with: export OPENAI_API_KEY=your_api_key")
        return

    # Initialize the YouTube transcript tool with OpenAI API enabled
    config = {
        "use_openai_api": True,
        "openai_api_key": openai_api_key,
        "openai_whisper_model": "whisper-1"
    }
    youtube_tool = YouTubeTranscriptTool(data_dir, config)

    # The video URLs to test
    video_urls = [
        "https://www.youtube.com/watch?v=49H4qtfUv24",  # F1 Japanese GP FP2 Highlights
        "https://www.youtube.com/watch?v=YXsOvs_eqQE"   # F1 interview with more likely transcript
    ]

    # Process each video
    for i, video_url in enumerate(video_urls):
        print(f"\n[{i+1}/{len(video_urls)}] Fetching transcript for video: {video_url}")

        # Extract the transcript
        transcript = youtube_tool.get_transcript(video_url)

        if transcript:
            print("\nTranscript found!")
            print(f"Video ID: {transcript['video_id']}")
            print(f"Language: {transcript['language']}")
            print(f"Fetched at: {transcript['fetched_at']}")
            print(f"Transcript length: {len(transcript['full_text'])} characters")
            print(f"Number of segments: {len(transcript['segments'])}")
            print(f"Source: {transcript.get('source', 'youtube')}")
            print(f"Is fallback: {transcript.get('is_fallback', False)}")

            # Print a sample of the transcript
            print("\nSample of transcript:")
            sample_length = min(500, len(transcript['full_text']))
            print(transcript['full_text'][:sample_length] + "...")

            # Save the transcript to a file
            output_file = os.path.join(output_dir, f"transcript_{transcript['video_id']}_openai.json")
            with open(output_file, 'w', encoding='utf-8') as f:
                json.dump(transcript, f, ensure_ascii=False, indent=2)

            print(f"\nFull transcript saved to: {output_file}")
        else:
            print("\nNo transcript found for this video.")

        print("\n" + "-"*80)

if __name__ == "__main__":
    main()
